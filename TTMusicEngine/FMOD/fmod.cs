...
namespace FMOD
{
    ...
    */
    public enum RESULT :int
    {
        ...
        ERR_BADCOMMAND,            /* Tried to call a function on a data type that does not allow this type of functionality (ie calling Sound::lock on a streaming _sound). */
        ...
        ERR_CHANNEL_STOLEN,        /* The specified channel has been reused to play another _sound. */
        ...
        ERR_INVALID_SYNCPOINT,     /* The syncpoint did not come from this _sound handle. */
        ...
        ERR_MAXAUDIBLE,            /* Reached maximum audible playback count for this _sound's soundgroup. */
        ...
        ERR_MEMORY_SRAM,           /* Not enough memory or resources on console _sound ram. */
        ERR_NEEDS2D,               /* Tried to call a command on a 3d _sound when the command was meant for 2d _sound. */
        ERR_NEEDS3D,               /* Tried to call a command on a 2d _sound when the command was meant for 3d _sound. */
        ERR_NEEDSHARDWARE,         /* Tried to use a feature that requires hardware support.  (ie trying to play a VAG compressed _sound in software on PS2). */
        ...
        ERR_NOTREADY,              /* Operation could not be performed because specified _sound is not ready. */
        ...
        ERR_OUTPUT_CREATEBUFFER,   /* Error creating hardware _sound buffer. */
        ...
        ERR_OUTPUT_NOHARDWARE,     /* FMOD_HARDWARE was specified but the _sound card does not have the resources nescessary to play it. */
        ERR_OUTPUT_NOSOFTWARE,     /* Attempted to create a software _sound but no software channels were specified in System::init. */
        ERR_PAN,                   /* Panning only works with mono or stereo _sound sources. */
        ...
        ERR_PRELOADED,             /* The specified _sound is still in use by the event system, call EventSystem::unloadFSB before trying to release it. */
        ERR_PROGRAMMERSOUND,       /* The specified _sound is still in use by the event system, wait for the event which is using it finish with it. */
        ...
        ERR_SUBSOUND_ALLOCATED,    /* This subsound is already being used by another _sound, you cannot have more than one parent to a _sound.  Null out the other parent's entry first. */
        ...
        ERR_SUBSOUND_MODE,         /* The subsound's mode bits do not match with the parent _sound's mode bits.  See documentation for function that it was called with. */
        ERR_SUBSOUNDS,             /* The error occured because the _sound referenced contains subsounds.  (ie you cannot play the parent _sound as a static sample, only its subsounds.) */
        ERR_TAGNOTFOUND,           /* The specified tag could not be found or there are no tags. */
        ERR_TOOMANYCHANNELS,       /* The _sound created exceeds the allowable input channel count.  This can be increased using the maxinputchannels parameter in System::setSoftwareFormat. */
    ...
    */
    public enum OUTPUTTYPE :int
    {
        ...
        NOSOUND,       /* All         - All calls in this mode succeed but make no _sound. */
    ...
    */
    ...
    [
        ...
        Note below the phrase '_sound channels' is used.  These are the subchannels inside a _sound, they are not related and 
        have nothing to do with the FMOD class "Channel".<br>
        For example a mono _sound has 1 _sound channel, a stereo _sound has 2 _sound channels, and an AC3 or 6 channel wav file have 6 "_sound channels".<br>
        <br>
        ...
        Sound channels map to speakers sequentially, so a mono _sound maps to output speaker 0, stereo _sound maps to output speaker 0 & 1.<br>
        ...
        Mono, stereo and multichannel sounds have each _sound channel played on the one speaker unity.<br>
        ...
        <li>Multichannel sounds have each _sound channel played on each speaker at unity.<br>
        ...
        <li>Stereo sounds default to the left _sound channel played on the front left, and the right _sound channel played on the front right.<br>
        <li>They can be cross faded with Channel::setPan.<br>
        <li>Multichannel sounds default to all of their _sound channels being played on each speaker in order of input.<br>
        <li>Mix behaviour for multichannel sounds can be set with Channel::setSpeakerLevels.<br>
        ...
        <li>Stereo sounds default to the left _sound channel played on the front left, and the right _sound channel played on the front right.  
        <li>They can be cross faded with Channel::setPan.<br>
        <li>Multichannel sounds default to all of their _sound channels being played on each speaker in order of input.<br>
        <li>Mix behaviour for multichannel sounds can be set with Channel::setSpeakerLevels.<br>
        ...
        <li>Stereo sounds default to the left _sound channel played on the front left, and the right _sound channel played on the front right.  
        <li>They can be cross faded with Channel::setPan.<br>
        <li>Multichannel sounds default to all of their _sound channels being played on each speaker in order of input.  
        <li>Mix behaviour for multichannel sounds can be set with Channel::setSpeakerLevels.<br>
        ...
        <li>Multichannel sounds default to all of their _sound channels being played on each speaker in order of input.  
        <li>Mix behaviour for multichannel sounds can be set with Channel::setSpeakerLevels.<br>
        <li>Channel::setSpeakerMix works and every parameter is used to set the balance of a _sound in any speaker.<br>
        <br>
    ...
    */
    public enum PLUGINTYPE :int
    {
        ...
        DSP         /* The plugin type is a DSP unit.  FMOD will use these plugins as part of its DSP network to apply effects to output or generate _sound in realtime. */
    }
    ...
    */
    public enum INITFLAGS :int
    {
        ...
        SOFTWARE_HRTF           = 0x00000010,   /* All platforms - All FMOD_SOFTWARE with FMOD_3D based voices will add a software lowpass filter effect into the DSP chain which causes sounds to _sound duller when the _sound goes behind the listener. */
        ...
        PS2_SWAPDMACHANNELS     = 0x00080000,   /* PS2 only - Changes FMOD from using SPU DMA channel 0 for software mixing, and 1 for _sound data upload/file streaming, to 1 and 0 respectively. */
        XBOX_REMOVEHEADROOM     = 0x00100000,   /* XBox only - By default DirectSound attenuates all _sound by 6db to avoid clipping/distortion.  CAUTION.  If you use this flag you are responsible for the final mix to make sure clipping / distortion doesn't happen. */
        ...
        GEOMETRY_USECLOSEST     = 0x04000000,   /* All platforms - With the geometry engine, only process the closest polygon rather than accumulating all polygons the _sound to listener line intersects. */
    ...
    */
    public enum SOUND_TYPE
    {
        ...
        DLS,             /* Sound font / downloadable _sound bank. */
    ...
    */
    public enum SOUND_FORMAT :int
    {
        ...
        MAX,      /* Maximum number of _sound formats supported. */ 
    ...
    [
        ...
        [REMARKS]
        By default a _sound will open as a static _sound that is decompressed fully into memory.<br>
        To have a _sound stream instead, use FMOD_CREATESTREAM.<br>
    ...
    */
    ...
    {
        DEFAULT                = 0x00000000,  /* FMOD_DEFAULT is a default _sound type.  Equivalent to all the defaults listed below.  FMOD_LOOP_OFF, FMOD_2D, FMOD_HARDWARE. */
        ...
        _3D                    = 0x00000010,  /* Makes the _sound positionable in 3D.  Overrides FMOD_2D. */
        HARDWARE               = 0x00000020,  /* Attempts to make sounds use hardware acceleration. (default). */
        SOFTWARE               = 0x00000040,  /* Makes _sound reside in software.  Overrides FMOD_HARDWARE.  Use this for FFT, DSP, 2D multi speaker support and other software related features. */
        ...
        OPENONLY               = 0x00002000,  /* Just open the file, dont prebuffer or read.  Good for fast opens for info, or when _sound::readData is to be used. */
        ...
        NONBLOCKING            = 0x00010000,  /* For opening sounds and getting streamed subsounds (seeking) asyncronously.  Use Sound::getOpenState to poll the state of the _sound as it opens or retrieves the subsound in the background. */
        UNIQUE                 = 0x00020000,  /* Unique _sound, can only be played one at a Time */
        _3D_HEADRELATIVE       = 0x00040000,  /* Make the _sound's position, velocity and orientation relative to the listener. */
        _3D_WORLDRELATIVE      = 0x00080000,  /* Make the _sound's position, velocity and orientation absolute (relative to the world). (DEFAULT) */
        _3D_LOGROLLOFF         = 0x00100000,  /* This _sound will follow the standard logarithmic rolloff model where mindistance = full volume, maxdistance = where _sound stops attenuating, and rolloff is fixed according to the global rolloff factor.  (default) */
        _3D_LINEARROLLOFF      = 0x00200000,  /* This _sound will follow a linear rolloff model where mindistance = full volume, maxdistance = silence.  */
        _3D_CUSTOMROLLOFF      = 0x04000000,  /* This _sound will follow a rolloff model defined by Sound::set3DCustomRolloff / Channel::set3DCustomRolloff.  */
        ...
        IGNORETAGS             = 0x02000000,  /* Skips id3v2/asf/etc tag checks when opening a _sound, to reduce seek/read overhead when opening files (helps with CD performance). */
        LOWMEM                 = 0x08000000,  /* Removes some features from samples to give a lower memory overhead, like Sound::getName. */
        LOADSECONDARYRAM       = 0x20000000,  /* Load _sound into the secondary RAM of supported platform.  On PS3, sounds will be loaded into RSX/VRAM. */
        VIRTUAL_PLAYFROMSTART  = 0x80000000   /* For sounds that start virtual (due to being quiet or low importance), instead of swapping back to audible, and playing at the correct offset according to Time, this flag makes the _sound play from the start. */
    }
    ...
    [
        [DESCRIPTION]   
        These values describe what state a _sound is in after NONBLOCKING has been used to open it.
    ...
    */
    ...
    {
        BEHAVIOR_FAIL,              /* Any _sound played that puts the _sound count over the SoundGroup::setMaxAudible setting, will simply fail during System::playSound. */
        BEHAVIOR_MUTE,              /* Any _sound played that puts the _sound count over the SoundGroup::setMaxAudible setting, will be silent, then if another _sound in the group stops the _sound that was silent before becomes audible again. */
        BEHAVIOR_STEALLOWEST        /* Any _sound played that puts the _sound count over the SoundGroup::setMaxAudible setting, will steal the quietest / least important _sound playing in the group. */
    }
    ...
    */
    ...
    {
        NOINTERP,        /* No interpolation.  High frequency aliasing hiss will be audible depending on the sample rate of the _sound. */
    ...
    [
        [DESCRIPTION]   
        List of tag types that could be stored within a _sound.  These include id3 tags, metadata from netstreams and vorbis/asf data.
    ...
    */
    ...
    {
        END_MS,              /* Delay at the end of the _sound in milliseconds.  Use delayhi only.   Channel::isPlaying will remain true until this delay has passed even though the _sound itself has stopped playing.*/
        DSPCLOCK_START,      /* Time the _sound started if Channel::getDelay is used, or if Channel::setDelay is used, the _sound will delay playing until this exact tick. */
        DSPCLOCK_END,        /* Time the _sound should end. If this is non-zero, the channel will go silent at this exact tick. */
        DSPCLOCK_PAUSE,      /* Time the _sound should pause. If this is non-zero, the channel will pause at this exact tick. */
    ...
    */
    public enum TIMEUNIT
    {
        ...
        RAWBYTES          = 0x00000008,  /* Raw file bytes of (compressed) _sound data (does not include headers).  Only used by Sound::getLength and Channel::getPosition. */
    ...
    [
        [DESCRIPTION]
        When creating a multichannel _sound, FMOD will pan them to their default speaker locations, for example a 6 channel _sound will default to one channel per 5.1 output speaker.<br>
        Another example is a stereo _sound.  It will default to left = front left, right = front right.<br>
        <br>
        This is for sounds that are not 'default'.  For example you might have a _sound that is 6 channels but actually made up of 3 stereo pairs, that should all be located in front left, front right only.
    ...
    */
    public enum SPEAKERMAPTYPE
    {
        ...
        ALLMONO,     /* This means the _sound is made up of all mono sounds.  All voices will be panned to the front center by default in this case.  */
        ALLSTEREO,   /* This means the _sound is made up of all stereo sounds.  All voices will be panned to front left and front right alternating every second channel.  */
        _51_PROTOOLS /* Map a 5.1 _sound to use protools L C R Ls Rs LFE mapping.  Will return an error if not a 6 channel _sound. */
    }
    ...
    [
        ...
        <li>To create a user created / non file based _sound.
        ...
        <li>To specify 'piggyback' read and seek callbacks for capture of _sound data as fmod reads and decodes it.  Useful for ripping decoded PCM data from sounds as they are loaded / played.
        <li>To specify a MIDI DLS/SF2 sample set file to load when opening a MIDI file.
        See below on what members to fill for each of the above types of _sound you want to create.
        ...
        <li>Create the _sound using the FMOD_OPENMEMORY flag.
        ...
        <u>To create a user created / non file based _sound.</u>
        <li>Create the _sound using the FMOD_OPENUSER flag.
        ...
        <u>To specify 'piggyback' read and seek callbacks for capture of _sound data as fmod reads and decodes it.  Useful for ripping decoded PCM data from sounds as they are loaded / played.</u>
    ...
    */
    ...
    public struct CREATESOUNDEXINFO
    {
        ...
        public uint                        length;                 /* [in] Optional. Specify 0 to ignore. Size in bytes of file to load, or _sound to create (in this case only if FMOD_OPENUSER is used).  Required if loading from memory.  If 0 is specified, then it will use the size of the file (unless loading from memory then an error will be returned). */
        ...
        public int                         numchannels;            /* [in] Optional. Specify 0 to ignore. Number of channels in a _sound specified only if OPENUSER is used. */
        public int                         defaultfrequency;       /* [in] Optional. Specify 0 to ignore. Default frequency of _sound in a _sound specified only if OPENUSER is used.  Other formats use the frequency determined by the file format. */
        public SOUND_FORMAT                format;                 /* [in] Optional. Specify 0 or SOUND_FORMAT_NONE to ignore. Format of the _sound specified only if OPENUSER is used.  Other formats use the format determined by the file format.   */
        public uint                        decodebuffersize;       /* [in] Optional. Specify 0 to ignore. For streams.  This determines the size of the double buffer (in PCM samples) that a stream uses.  Use this for user created streams if you want to determine the size of the callback buffer passed to you.  Specify 0 to use FMOD's default size which is currently equivalent to 400ms of the _sound format created/loaded. */
        ...
        public int                         numsubsounds;           /* [in] Optional. Specify 0 to ignore or have no subsounds.  In a user created multi-sample _sound, specify the number of subsounds within the _sound that are accessable with Sound::getSubSound / SoundGetSubSound. */
        ...
        public SOUND_PCMREADCALLBACK       pcmreadcallback;        /* [in] Optional. Specify 0 to ignore. Callback to 'piggyback' on FMOD's read functions and accept or even write PCM data while FMOD is opening the _sound.  Used for user sounds created with OPENUSER or for capturing decoded data as FMOD reads it. */
        public SOUND_PCMSETPOSCALLBACK     pcmsetposcallback;      /* [in] Optional. Specify 0 to ignore. Callback for when the user calls a seeking function such as Channel::setPosition within a multi-sample _sound, and for when it is opened.*/
        public SOUND_NONBLOCKCALLBACK      nonblockcallback;       /* [in] Optional. Specify 0 to ignore. Callback for successful completion, or error while loading a _sound that used the FMOD_NONBLOCKING flag.*/
        ...
        public IntPtr                      userdata;               /* [in] Optional. Specify 0 to ignore. This is user data to be attached to the _sound during creation.  Access via Sound::getUserData. */
        ...
        public IntPtr                      initialsoundgroup;      /* [in] Optional. Specify 0 to ignore. Specify a _sound group if required, to put _sound in as it is created. */
    ...
    */
    ...
    [
        ...
        If the _sound card does not support EAX4, the environment flag is ignored.
    ...
    */
    ...
    {                                      /*          MIN     MAX    DEFAULT  DESCRIPTION */
        ...
        public int       OutsideVolumeHF;      /* [in/out] -10000, 0,     0,       outside _sound cone level at high frequencies (win32) */
    ...
    */
    ...
    {                       
        ...
        public float   HRTFMinAngle;                /* [in/out] For use with FMOD_INIT_SOFTWARE_HRTF.  The angle (0-360) of a 3D _sound from the listener's forward vector at which the HRTF function begins to have an effect.  Default = 180.0. */
        public float   HRTFMaxAngle;                /* [in/out] For use with FMOD_INIT_SOFTWARE_HRTF.  The angle (0-360) of a 3D _sound from the listener's forward vector at which the HRTF function begins to have maximum effect.  Default = 360.0.  */
        ...
        public float   vol0virtualvol;              /* [in/out] For use with FMOD_INIT_VOL0_BECOMES_VIRTUAL.  If this flag is used, and the volume is 0.0, then the _sound will become virtual.  Use this value to raise the threshold to a different point where a _sound goes virtual. */
